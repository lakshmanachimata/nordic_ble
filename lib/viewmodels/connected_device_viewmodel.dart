...
class ConnectedDeviceViewModel extends ChangeNotifier {
  ...
  bool get hasCompletedAllCommands => _commandResponses.length == commands.length;
  ...
  Future<void> startCommandSequence() async {
    ...
    try {
      ...
      for (int i = 0; i < commands.length; i++) {
        ...
        await _writeCharacteristic!.write(utf8.encode(encodedCommand));
        ...
        final response = await _waitForResponse(timeout: const Duration(seconds: 10));
        
        // Add command response
        _commandResponses.add(CommandResponse(
          command: command,
          response: response,
